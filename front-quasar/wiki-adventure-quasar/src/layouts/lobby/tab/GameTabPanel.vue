<template>
  <div id="gameTab">
    ...
    <q-separator  spaced="lg"/>
    <div id="vote" class="row items-baseline q-ma-sm"><p class="q-ma-none col-strech">{{ $t('gameTab.vote') }} :</p><p class="q-ma-none q-ml-sm text-left col-grow">{{ vote }}</p><q-btn round dense flat icon="mdi-delete" class="voteDelete" @click="resetVote()"></q-btn></div>
    ...
    <q-separator spaced="lg"/>
    <div class="row items-baseline q-ma-sm"><p class="q-ma-none col-strech">{{ $t('gameTab.startPage') }} :</p><p class="q-ma-none q-ml-sm text-left col-grow">{{ startPage }}</p></div>
    <div class="row items-baseline q-ma-sm"><p class="q-ma-none col-strech">{{ $t('gameTab.endPage') }} :</p><p class="q-ma-none q-ml-sm text-left col-grow">{{ endPage }}</p></div>
    <q-separator  spaced="lg"/>
    <div class="row items-baseline justify-evenly"><q-btn class="action-btn q-ma-sm" push :label="$t('gameTab.pageHistory')" icon="mdi-format-list-bulleted" @click="open('page-history')"/><q-btn class="action-btn q-ma-sm" push :label="$t('gameTab.showEndPage')" icon="mdi-page-next-outline" @click="open('wiki-end-page')"/></div>
    <q-separator  spaced="lg"/>
    <div class="row items-baseline justify-evenly"><p>{{ totalVoteSkip }} / {{ connectedPlayers }}</p><q-btn class="action-btn q-ma-sm" push label="skip" icon="mdi-skip-forward" @click="voteSkip()"/></div>
    <q-separator spaced="lg"/>
  </div>
</template>
<style lang="scss">
