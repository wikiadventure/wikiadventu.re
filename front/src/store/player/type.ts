 ...
 */
export type PlayerData<T extends  GameLoopType | unknown> = T extends VanillaLoopType.Classic ? unknown : 
                                                T extends VanillaLoopType.Random ? unknown : unknown;

/**
 * Player extends custom data depending on the gameloop
 */
export type Player<T extends  GameLoopType | unknown> = PlayerData<T> & {
    pseudo: string,
    id: number,
    score: number,
    voteSkip:boolean,
